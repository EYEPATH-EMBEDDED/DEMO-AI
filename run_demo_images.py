...
def run_demo_images(img_dir: str | Path, output_path: str | Path,
    ...
    for frame_idx, path in enumerate(img_paths):
        ...
        # 출력 & 프리뷰
        out.write(frame)
        cv2.imshow("Collision Predictor (images)", frame)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            _LOGGER.info("Interrupted by user – early exit.")
            break

    out.release(); cv2.destroyAllWindows()
    _LOGGER.info("Finished. Saved → %s (%.1f fps)",
                 output_path, len(img_paths)/(time.time()-t0))

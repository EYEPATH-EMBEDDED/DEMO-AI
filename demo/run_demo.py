...
def run_demo(video_path: str | Path, output_path: str | Path) -> None:
    ...
    predictor = CollisionPredictor(
        model_path="assets/first_best_model.pth", threshold=0.55
    )

    fps_orig = cap.get(cv2.CAP_PROP_FPS) or 30.0
    target_fps = 20
    skip = max(int(round(fps_orig / target_fps)), 1)   # 몇 프레임 건너뛸지
    frame_idx = 0
    t0 = time.time()
    while True:
        ret, frame = cap.read()
        ...
        if frame_idx % skip != 0:        # 20 fps 로 샘플링
            frame_idx += 1
            continue

        # ---------------------------------------------
        bboxes = detect_bboxes(frame, weights_path="assets/last.pt")
        prob = predictor.update(
            frame_idx = frame_idx // skip,        # queue index = 20fps 기준
            time_sec  = (frame_idx // skip) / target_fps,
            bboxes_xyxy = bboxes,
            img_w = w,
            img_h = h,
        )
